...
GeneratorInfdev20100611::GeneratorInfdev20100611(int64_t pSeed, float multiplier) : Generator(pSeed, multiplier) {
	...
	noiseGen1 = NoiseOctavesPerlin(rand, 16, 16, false);
	noiseGen2 = NoiseOctavesPerlin(rand, 16, 16, false);
	noiseGen3 = NoiseOctavesPerlin(rand,  8,  8, false);
	noiseGen4 = NoiseOctavesPerlin(rand,  4,  4, false);
	noiseGen5 = NoiseOctavesPerlin(rand,  4,  4, false);
	noiseGen6 = NoiseOctavesPerlin(rand, 10, 10, false);
	noiseGen7 = NoiseOctavesPerlin(rand, 16, 16, false);
	//mobSpawnerNoise = NoiseOctavesPerlin(rand, 8, 8, false);
}
...
Chunk GeneratorInfdev20100611::GenerateChunk(Int2 chunkPos) {
	...
	noiseGen6.GenerateOctaves(noise6, chunkX, 0, chunkZ, sampleWidth, 1, sampleWidth, 1.0, 0.0, 1.0);
	noiseGen7.GenerateOctaves(noise7, chunkX, 0, chunkZ, sampleWidth, 1, sampleWidth, 100.0, 0.0, 100.0);
	...
	for (int32_t noiseX = 0; noiseX < sampleWidth; ++noiseX) {
		for (int32_t noiseZ = 0; noiseZ < sampleWidth; ++noiseZ) {
			...
			for (int32_t noiseY = 0; noiseY < sampleHeight; ++noiseY) {
				// Vertical density falloff: centres around var65, scaled by n6
				double densityOffset = (double(noiseY) - var65) * 12.0 / n6;
