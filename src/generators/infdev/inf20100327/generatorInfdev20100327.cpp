...
GeneratorInfdev20100327::GeneratorInfdev20100327(int64_t pSeed, float multiplier) : Generator(pSeed, multiplier) {
	...
	noiseGen1 = NoiseOctavesPerlin(rand, 16,16);
	noiseGen2 = NoiseOctavesPerlin(rand, 16,16);
	noiseGen3 = NoiseOctavesPerlin(rand, 8 ,8);
	noiseGen4 = NoiseOctavesPerlin(rand, 4 ,4);
	noiseGen5 = NoiseOctavesPerlin(rand, 4 ,4);
	//noiseGen6 = NoiseOctavesPerlin(rand, 5 ,5); // Unused
	//mobSpawnerNoise = NoiseOctavesPerlin(rand, 5, 5);
}
	...
	for (int32_t pz = 0; pz < samples; ++pz) {
		for (int32_t px = 0; px < samples; ++px) {
			...
			if (SurfaceOctaveBudget(4, stride) > 0) {
				const double nX = double(originX + px * stride);
				const double nZ = double(originZ + pz * stride);
